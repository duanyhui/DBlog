package atri.wiki.util.telegram;

import atri.wiki.config.properties.BlogProperties;
import atri.wiki.config.properties.TelegramProperties;
...
import atri.wiki.util.comment.CommentUtils;
...
public class TelegramBotMsgHandler {
	...
	private String replyComment(String msgContent) {
		...
		if (splitIndex2 != -1 && msgContent.length() > splitIndex2 + 1) {
			...
			//先找到要回复的评论
			atri.wiki.entity.Comment parentComment = commentService.getCommentById(commentId);
